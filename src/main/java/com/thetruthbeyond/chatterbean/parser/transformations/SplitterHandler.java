...
public class SplitterHandler extends DefaultHandler {
	...
	private final List<String> splitters = new ArrayList<>(8);
	private final List<String> separator = new ArrayList<>(4);

	@Override
	public void startElement(String namespace, String name, String qualifiedName, Attributes attributes) throws SAXException {
		if(qualifiedName.equals("splitter") && attributes.getValue("type").equals("sentence"))
			splitters.add(attributes.getValue(0));
		else

		if(qualifiedName.equals("splitter") && attributes.getValue("type").equals("word"))
			separator.add(attributes.getValue(0));
	}
